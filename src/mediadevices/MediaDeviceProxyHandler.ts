...
export default class MediaDeviceProxyHandler implements ProxyHandler<MediaDevices> {
  ...
  constructor() {
    this.isDeviceChangeSupported = 'ondevicechange' in navigator.mediaDevices;
  }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private patchAddEventListener = (target: MediaDevices, property: PropertyKey, receiver: any) => {
    const value = Reflect.get(target, property, receiver);
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private patchRemoveEventListener = (target: MediaDevices, property: PropertyKey, receiver: any) => {
    const value = Reflect.get(target, property, receiver);
  ...
  private async handleDeviceChangeEvent() {
